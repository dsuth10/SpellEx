...
export default function AssessmentPage() {
  ...
  React.useEffect(() => {
    const savedUser = window.localStorage.getItem('spellex-user');
    if (savedUser) {
  ...
  const handleFinish = async () => {
    ...
    if (result.success) {
      ...
      alert("Error saving results to database. Your progress is still saved locally.");
    }
    ...
    return (
      <div className="min-h-screen bg-indigo-50 flex items-center justify-center p-4">
        <div className="max-w-2xl w-full bg-white rounded-[40px] shadow-2xl p-12 text-center border-b-[12px] border-emerald-500">
          ...
          <p className="text-slate-500 text-xl mb-12">Great job, {student?.username}! Your results are saved.</p>
          ...
          <div className="space-y-4">
            <Link
              href="/"
              className="block w-full py-4 bg-indigo-600 text-white rounded-2xl font-bold text-xl hover:bg-indigo-700 transition"
            >
              Back to Home
            </Link>
          </div>
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-[32px] shadow-xl overflow-hidden border border-slate-100">
        <header className="bg-indigo-900 p-8 text-white flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="text-center md:text-left">
            ...
            <p className="text-indigo-300 font-medium mt-1">{student?.username || "Student"}</p>
          </div>
        ...
        <footer className="bg-slate-50 border-t border-slate-200 p-8 flex flex-col md:flex-row justify-between items-center gap-8">
          ...
          {currentPart < 5 ? (
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 px-10 py-4 bg-emerald-600 text-white rounded-2xl font-black text-lg hover:shadow-xl hover:shadow-emerald-200 hover:-translate-y-0.5 transition active:translate-y-0 w-full md:w-auto disabled:opacity-50"
            >
              {isSubmitting ? "Saving..." : "Finish Assessment"} <CheckCircle size={20} />
            </button>
          )}
        </footer>
      </div>
